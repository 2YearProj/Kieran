...
PROJECT_ID='y2proj-messaging-c76a3'
COLLECTION_ID='messages'
API_KEY='AIzaSyC6xiSjSEQ91o3Co9S2SP6j1WmlFOXwCDw'
TOKEN=''
...
do
	...
	# so we parse out the msg and the actual col3 last part of the topic
        column4=`echo $line|cut -f4 -d/`
        msg=`echo $column4|cut -f2- -d' '`
        column4=`echo $column4|cut -f1 -d' '`
        echo $column4
        ...
        JSON='{"fields": {"createdAt": {"stringValue": "'"$now"'"}, "sender": {"stringValue": "'"$column4"'"},
          "recipient": {"stringValue": "'"$column5"'"}, "message": {"stringValue": "'"$column6"'"}, "jockey": {"stringValue": "'"$column7"'"}}}'
